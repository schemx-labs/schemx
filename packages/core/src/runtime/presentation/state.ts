 ...
 * 层级呈现状态。
 *
 * Group、Dependency 和 Field 共享 visible、readonly、disabled 的继承规则。
 *
 * @module core/runtime/presentation/state
 */
...
export interface PresentationRuntimeState {
   ...
   * 当前容器的静态呈现状态 signal。
   */
  readonly staticState: Signal<PresentationStaticState>
 ...
 * 创建容器运行时状态的配置。
 */
export interface CreatePresentationRuntimeStateOptions {
  /**
 ...
 */
export function createPresentationRuntimeState(
  options: CreatePresentationRuntimeStateOptions
): PresentationRuntimeState {
  // 创建可被静态 Schema 更新的容器状态 signal。
  const staticState = createSignal(options.staticState ?? DEFAULT_PRESENTATION_STATE, {
    name: `presentation:${options.nodeId}:staticState`,
  })
  ...
  const effectiveState = createComputed<PresentationState>(() => {
    ...
    return resolvePresentationState(current, overrides, inherited)
  })
...
export function createInheritedPresentationState<TValues extends Values>(
  ...
  return createComputed(() => readInheritedPresentationState(node.parent))
}

/** 没有静态配置或祖先状态时使用的默认呈现状态。 */
...
function readInheritedPresentationState<TValues extends Values>(
  ...
  while (current && current.type !== "root") {
    if (current.presentationState) {
      return current.presentationState.effectiveState.value
    }
 ...
 * 解析节点的最终呈现状态。
 *
 * 动态覆盖优先于静态值，静态值缺失时回退默认值；结果始终受祖先状态约束。
 */
