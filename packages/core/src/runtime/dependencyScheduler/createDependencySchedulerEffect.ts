 ...
 * 依赖任务调度 effect 创建器。
 *
 * @module core/runtime/dependencyScheduler/createDependencySchedulerEffect
 */
 ...
 */
export interface DependencySchedulerEffect<TResult> {
  /**
   * 立即执行一次依赖任务。
   *
 ...
 * @typeParam TResult - 依赖任务成功完成时产生的结果类型。
 */
export interface CreateDependencySchedulerEffectOptions<TValues extends Values, TResult> {
  /**
 ...
 */
export function createDependencySchedulerEffect<TValues extends Values, TResult>(
  options: CreateDependencySchedulerEffectOptions<TValues, TResult>
): DependencySchedulerEffect<TResult> {
  ...
  if (options.triggerFields.length > 0) {
    ...
    const disposeSubscription = createSignalEffect(() => {
      void formApi.getFieldsValue([...options.triggerFields])
