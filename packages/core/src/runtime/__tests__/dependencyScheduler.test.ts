...
function createTestFormApi(
  ...
  return {
    getFieldValue: store.getFieldValue.bind(store),
    getFieldsValue: store.getFieldsValue.bind(store),
...
describe("createDependencySchedulerEffect", () => {
  it("应只订阅 triggerFields，并在非追踪上下文执行任务", async () => {
    ...
    const run = vi.fn(() => {
      void formApi.getFieldValue("city")
