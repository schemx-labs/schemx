...
export interface SchemxInstance<TValues extends Values = Values> {
   ...
   * @example
   * ```typescript
   * const entry = form.getPresetRule('phone')
   * ```
   */
  getPresetRule: (name: string) => PresetRuleEntry | undefined

  /**
   * 注册校验规则
   *
   * 将校验规则注册到当前表单的 PresetRuleRegistry，
   * 后续可通过 getPresetRule / hasPresetRule 查询。
   *
   * @param name - 规则名称
   ...
   * @example
   * ```typescript
   * form.registerPresetRule('phone', phoneRule)
   * ```
   */
  registerPresetRule: (
    name: string,
    rule: PresetRuleEntry,
   ...
   * @example
   * ```typescript
   * form.hasPresetRule('phone') // => true
   * ```
   */
  hasPresetRule: (name: string) => boolean
