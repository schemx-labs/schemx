...
Form 使用 `row` 配置根级 Row；Group 和 Dynamic Schema 使用 `row` 配置各自的子级 Row，Dynamic 会为每个数组 item 创建一行。Field 使用 `col` 配置自身的 Col：
...
旧的 `layout` 字段以及 `SchemxLayout` 类型进入弃用期。兼容期内 Field 的 `col` 优先于旧 `layout`；新代码应迁移到 `row` / `col`。
