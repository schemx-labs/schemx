...
# 工作区目标发现与选择。目标来自 packages、plugins、examples 下的 package.json。

workspace_module_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "$workspace_module_dir/../../lib/package-json.sh"
source "$workspace_module_dir/../../lib/workspace-discovery.sh"
...
workspace_discover_task_targets() {
  ...
  while IFS=$'\t' read -r scope directory package_name package_file; do
    ...
    for candidate in "${candidates[@]}"; do
      if lib_package_has_script "$package_file" "$candidate"; then
  ...
  done < <(lib_discover_workspace_targets "$root_dir" packages plugins examples)
}

# 将一行一个目标标识转换为逗号分隔列表，供 WORKFLOW_TARGETS 和 CLI 统一消费。
workspace_join_targets() {
  local value
  local result=''
...
workspace_discover_targets() {
  ...
  while IFS= read -r rule; do
    ...
    while IFS=$'\t' read -r _ directory package_name package_file; do
      if [[ "$script" != '*' ]] && ! lib_package_has_script "$package_file" "$script"; then
        continue
      fi
      printf '%s\t%s\t%s\t%s\n' "$scope" "$directory" "$package_name" "$output_script"
    done < <(lib_discover_workspace_targets "$root_dir" "$scope")
  done < <(tr ',' '\n' <<< "$rules")
}

# 返回目录分类在树形交互控件中使用的名称，保持目录层级的可辨识性。
workspace_scope_prompt_label() {
  local scope="$1"
...
workspace_grouped_target_options() {
  local records="$1"
  local scope
  local scope_label
  local directory
...
workspace_select_target_identifiers() {
  ...
  if [[ "$mode" == single ]]; then
    ...
    grouped_options="$(workspace_grouped_target_options "$records")"
  ...
  workspace_join_targets <<< "$selected"
}
...
workspace_select_task_targets() {
  ...
  if [[ -z "$selected" ]] && _ui_is_interactive; then
