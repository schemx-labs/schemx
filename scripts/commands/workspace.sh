...
# 将可缓存的标准任务交给 Turborepo 处理依赖顺序与缓存；非标准脚本保持直接执行。
workspace_run_target() {
  ...
  case "$script" in
    build | lint | type-check | test)
      ui_task --title "${label} ${package_name}" --log live -- pnpm exec turbo run "$script" "--filter=${package_name}" || return
      ;;
    *)
      ui_task --title "${label} ${package_name}" --log live -- pnpm --filter "$package_name" run "$script" || return
      ;;
  esac
}
...
workspace_run() {
  ...
  ui_flow_begin --domain workspace --title "$label" --description '由 Turborepo 编排可缓存任务，其余脚本保持直接执行。' || return
  ...
  while IFS=$'\t' read -r scope directory package_name script; do
    ...
    ((count += 1))
    workspace_run_target "$task" "$package_name" "$script" "$label" || {
      local exit_code=$?
