...
release_execute_plan() {
  ...
  if [[ "$channel" == latest && "$version_action" != current ]]; then
    while IFS=$'\t' read -r package package_name version tag; do
      [[ -n "$package" ]] || continue
      display_command="$(_ui_command_text npm --prefix "packages/$package" version "$version" --no-git-tag-version)" || return
      ui_task --title "写入 ${package_name}@${version}" --display "$display_command" --log live -- bash "$workflow_root/scripts/modules/release/runner.sh" write-package-version "$package" "$version" || return
    ...
    display_command="$(_ui_command_text git add -- "${package_files[@]}") && $(_ui_command_text git commit -m 'chore(发布): 更新正式版本')" || return
    ui_task --title '提交正式版版本变更' --display "$display_command" --log live -- bash "$workflow_root/scripts/modules/release/runner.sh" commit-release-version 'chore(发布): 更新正式版本' "${package_files[@]}" || return
    ...
    while IFS=$'\t' read -r package package_name version tag; do
      ...
      display_command="$(_ui_command_text npm --prefix "packages/$package" version "$version" --no-git-tag-version)" || return
      ui_task --title "写入临时 ${package_name}@${version}" --display "$display_command" --log live -- bash "$workflow_root/scripts/modules/release/runner.sh" write-package-version "$package" "$version" || return
    done < <(release_plan_release_records "$plan_file")
  fi
  ...
  if [[ "$channel" != dev ]]; then
    if [[ "$channel" == latest ]]; then
      display_command="$(_ui_command_text git push origin HEAD)" || return
      ui_task --title '推送正式版提交' --display "$display_command" --log live -- bash "$workflow_root/scripts/modules/release/runner.sh" push-commit || return
    fi
