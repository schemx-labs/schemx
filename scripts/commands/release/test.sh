...
release_test() {
  ...
  while IFS=$'\t' read -r label test_file; do
    ...
    display_command="$(_ui_command_text bash "$test_file")" || return
    ui_task --title "测试 ${label}" --display "$display_command" --log live -- bash "$workflow_root/scripts/modules/release/tests.sh" run bash "$workflow_root/$test_file" || { local exit_code=$?; ui_flow_end failed "发布脚本测试失败：${label}。"; return "$exit_code"; }
