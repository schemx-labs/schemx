...
tools_run() {
  ...
  case "$command" in
    ...
    check:packages)
      ...
      ui_task --title '检查 workspace 包配置' --log live -- bash scripts/modules/packages/check-config.sh || { local exit_code=$?; ui_flow_end failed '包配置检查失败。'; return "$exit_code"; }
      ui_task --title '检查构建产物 external 边界' --log live -- bash scripts/modules/packages/check-bundle-boundaries.sh || { local exit_code=$?; ui_flow_end failed '构建产物边界检查失败。'; return "$exit_code"; }
  ...
  if [[ "$command" == 'pack-local' ]]; then
    ...
    set -- env "SCHEMX_WORKFLOW_TARGETS=${target}" bash scripts/modules/packages/pack-local.sh
  fi
