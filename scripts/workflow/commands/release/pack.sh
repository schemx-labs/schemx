...
release_pack() {
  ...
  while IFS= read -r package; do
    [[ -n "$package" ]] || continue
    ui_task --title "生成 $(targets_package_name "$package") tarball" --log live -- bash "$workflow_root/scripts/workflow/domains/release/runner.sh" pack-artifact "$(targets_package_name "$package")" "$destination" || { local exit_code=$?; ui_flow_end failed "本地 tarball 生成失败：$(targets_package_name "$package")。"; return "$exit_code"; }
  done <<< "$packages"
