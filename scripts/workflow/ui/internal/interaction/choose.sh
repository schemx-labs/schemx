...
ui_prompt() {
  ...
  case "$kind" in
    select | multiselect)
      while [[ $# -gt 0 ]]; do
        case "$1" in
          ...
          --option) [[ $# -ge 3 ]] || { ui__write_stderr 'ui_prompt 用法错误：--option 需要 value 和 label。'; return 2; }; options+=("$2:::$3"); shift 3 ;;
    ...
    group-multiselect)
      while [[ $# -gt 0 ]]; do
        case "$1" in
          --message) message="${2:-}"; shift 2 ;;
          --group) [[ $# -ge 3 ]] || { ui__write_stderr 'ui_prompt 用法错误：--group 需要 id 和 label。'; return 2; }; groups+=("group:::${2}:::${3}"); shift 3 ;;
          --option) [[ $# -ge 4 ]] || { ui__write_stderr 'ui_prompt 用法错误：--option 需要 group、value 和 label。'; return 2; }; groups+=("${2}:::${3}:::${4}"); shift 4 ;;
  ...
  if [[ "$kind" == group-multiselect ]]; then
    [[ "${#groups[@]}" -gt 0 ]] || { ui__write_stderr 'ui_prompt 用法错误：至少需要一个 --group 或 --option。'; return 2; }
  fi
